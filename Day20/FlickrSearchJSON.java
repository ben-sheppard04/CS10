...
public class FlickrSearchJSON extends JFrame {
	...
	public FlickrSearchJSON() {
		...
		canvas = new JComponent() {
			public void paintComponent(Graphics g) { //called on repaint()
				...
				if (images.size() > 0) {
					g.drawImage(images.get(curr), 0, 0, null); // Draw image on frame
				}
			}
		};
	...
	private JPanel setupGUI() {
		...
		nextB.addActionListener(new AbstractAction() {
			public void actionPerformed(ActionEvent e) {
				if (images.size() > 0) {
					curr = (curr + 1) % images.size(); // prevents from overflowing
					repaint();
				}
			}
		});
		...
		search.addActionListener(new AbstractAction() {
			public void actionPerformed(ActionEvent e) {
				...
				try {
					loadImages(queryF.getText()); // loadImages passes in text you typed in.
					curr = 0;
					repaint();
				}
				catch (Exception ex) {
	...
	private void loadImages(String query) throws Exception {
		...
		// Read JSON response from Flickr and store in String str. Just like reading file
