...
public class RegionFinder {
	...
	public void findRegions(Color targetColor) {
		...
		protected static boolean colorMatch (Color c1, Color c2){
			...
			if (Math.abs(redC1 - redC2) > maxColorDiff) { //Checks if the two red components are similar enough to within maxColorDiff
				return false; //returns false if they are not sufficiently close
			}
			if (Math.abs(greenC1 - greenC2) > maxColorDiff) { //Checks if the two green components are similar enough to within maxColorDiff
				return false; //returns false if they are not sufficiently close
			}
            return Math.abs(blueC1 - blueC2) <= maxColorDiff; //Checks if the two blue components are similar enough to within maxColorDiff, returns true if they are.
        }
