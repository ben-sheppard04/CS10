...
public class WebcamColorTracking extends VideoGUI {
	...
	private Point track() {
		...
		for (int y = 0; y < image.getHeight(); y++) {
			for (int x = 0; x < image.getWidth(); x++) {
				...
				int d = (c.getRed() - trackColor.getRed()) * (c.getRed() - trackColor.getRed()) //squares it to always get pos number (could also do abs)
		...
		return new Point(cx,cy); //returns a Point object
	}
	 ...
	 */
	@Override
	public void handleMousePress(int x, int y) { //save culor under mouse pointer in instance variable trackColor(if camera runs)
	...
	public void handleImage() {
		...
		if (trackColor != null) {
			...
			Point p = track(); //java provides Point class. Has x and y instance variable.
