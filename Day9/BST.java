...
public class BST<K extends Comparable<K>,V> { // extends an interface called Comparable, which says you must implement compareTo().
	//CompareTo is already written for autoboxed classes like Integer and String. if you want to implement our own class
	//you must implement your own.
	private K key; //Generic - K for key. Need some way to know which key is greater than or smaller than
	private V value; //Generic - V for value
	private BST<K,V> left, right; //type BST
	 ...
	 * Can take no more than O(h) checks
	 * Warning! No balance guarantee so cant say O(log2n); could be a vine.
	 */
	public V find(K search) throws InvalidKeyException { //return value of type V. O(h)
