...
implements MinPriorityQueue<E> {
	...
	public void insert(E element) {
		...
		while (loc > 0 && heap.get(loc).compareTo(heap.get(parent(loc))) < 0) { //compare it to the parent. For max priority queue, make it >
			swap(heap, loc, parent(loc));
			loc = parent(loc); //update location to be the parent
		}
	}
