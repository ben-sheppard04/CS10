...
public class DotTreeGUI extends InteractiveGUI {
	...
	public void handleMousePress(int x, int y) {
		if (mode == 'a') {
			if (tree == null) {
				tree = new PointQuadtree<Point>(new Point(x,y), 0,0, width, height); // Make a new quadtree if tree is null

			} else {
				tree.insert(new Point(x,y)); // Add a new Point at x,y in the quadtree if tree is not null

			}
		} else if (mode == 'q') {
			found = tree.findInCircle(x, y, mouseRadius); // Set "found" to what tree says is near the mouse press
		} else {
	...
	/**
	 * Test tree 2 -- Creates test tree as explained in writeup.
	 */
	private void test2() {
		found = null;
		tree = new PointQuadtree<Point>(new Point(400,300), 0,0,800,600); //New quadtree with root at center of window.
		tree.insert(new Point(200,150));
		tree.insert(new Point(100,75));
		tree.insert(new Point(50,37));

		tree.insert(new Point(200,450));
		tree.insert(new Point(100,525));
		tree.insert(new Point(50,562));

		tree.insert(new Point(600,150));
		tree.insert(new Point(700,75));
		tree.insert(new Point(750,37));

		tree.insert(new Point(600,450));
		tree.insert(new Point(700,525));
		tree.insert(new Point(750,562));


		tree.insert(new Point(450,100));
		tree.insert(new Point(500,75));
		tree.insert(new Point(550,50));

		tree.insert(new Point(440,115));
		tree.insert(new Point(510,50));
		tree.insert(new Point(555,45));
	}
	...
	public void handleKeyPress(char key) {
		if (key=='a') {
			...
			test2(); // Runs test2 if '2' key is pressed.
		}
		repaint(); //erase the window and call draw
	}
	...
	public void drawTree(Graphics g, PointQuadtree<Point> tree, int level) {
		...
		if(tree.hasChild(1))
			drawTree(g, tree.getChild(1), level+1); // level is one lower than previous, runs it on child tree c1
		if(tree.hasChild(2))
			drawTree(g, tree.getChild(2), level+1); // level is one lower than previous, runs it on child tree c2
		if(tree.hasChild(3))
			drawTree(g, tree.getChild(3), level+1); // level is one lower than previous, runs it on child tree c3
		if(tree.hasChild(4))
			drawTree(g, tree.getChild(4), level+1); // level is one lower than previous, runs it on child tree c4
