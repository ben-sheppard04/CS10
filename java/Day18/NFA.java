...
public class NFA {
	...
	// note the difference from DFA: can have multiple different transitions from state for character. Now its a List of Strings not String.
	...
	public boolean match(String s) {
		// difference from DFA: multiple current states so it's a set.
		Set<String> currStates = new TreeSet<String>();
		currStates.add(start);
		for (int i=0; i<s.length(); i++) {
			char c = s.charAt(i);
			Set<String> nextStates = new TreeSet<String>(); // States you might be in
