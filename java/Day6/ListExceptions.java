...
public class ListExceptions {
	public static void main(String[] args) { // note: no "throws exception", as every method that could is wrapped
		...
		try { //try to run this code, and if exception happens, jump to the catch bar, where it is handled
			list.add(-1, "?");
		...
		try {
			...
			System.out.println(e); 					//will give us the error message using e (prints exception in the implemenets)
		}
		finally {
			System.out.println("finally 1"); // executed whether or not code caught an error
		}
