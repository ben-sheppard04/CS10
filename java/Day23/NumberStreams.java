...
public class NumberStreams {
	public static void main(String[] args) throws Exception {		
		...
		IntStream.range(1, 10)
				 .filter(i -> i%2 == 0)
				 .map(i -> i*3) // example of laziness because you put map after filter, so you save computation
				 .forEach(System.out::println);
		System.out.println("<enter> for example "+(ex++)); console.nextLine();
		...
		IntStream.range(0,10)
			     ...
			     .limit(3) //demand for 3 items, previous won't produce unless consumer asks it for, so only first three processed Short circuit
		...
		IntStream.range(1, 10)
				 .parallel()
				 .filter(i -> { System.out.println("f "+i); return i%2 == 0; }) // multi line lambda fn
