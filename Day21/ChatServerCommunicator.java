...
public class ChatServerCommunicator extends Thread {
	...
	public void run() {
		try {
			...
			name = in.readLine(); // ASK-- HOW DO YOU KNWO THAT THE USER HAS TYPED NAME ALREADY???? DOes it block here?
			...
			while ((line = in.readLine()) != null) {
				...
				server.broadcast(this, msg);  // Need this to check for when broadcasting which one sent it so you don't broadcast to that one.
			}
